...
if(localStorage.getItem("quick_links") != "false"){
...
function updateLinks() {
    ...
    for(var i=0;i<this.links.length;i++){
        ...
        this.newLink.onclick = function () {
            ...
            window.location.href = '/math/' + this.id; 
        }
    }
}
...
openLinks.querySelector(".add").onclick = function(){
    ...
    if (!isUrl(this.quickLinkUrl)) this.quickLinkUrl = (localStorage.getItem("search_engine") == "ggl" ? 'https://www.google.com/search?q=' : 'https://duckduckgo.com/?q=') + this.quickLinkUrl;
...
customTab.onclick = function() {
    ...
    if (!(this.tabIcon.startsWith('https://') || this.tabIcon.startsWith('http://'))) this.tabIcon = 'https://' + this.tabIcon;
...
if(localStorage.getItem("search_engine")){
    search_engine.value = localStorage.getItem("search_engine");
}

document.querySelector("#selected-engine").innerText = search_engine.options[search_engine.selectedIndex].innerText;

search_engine.onchange = () => {
    localStorage.setItem("search_engine", search_engine.value);
    document.querySelector("#selected-engine").innerText = search_engine.options[search_engine.selectedIndex].innerText;
}
