importScripts('/classes/balance.js');
importScripts('/classes/calculus.js');

class UVServiceWorker extends EventEmitter {     
    constructor(config = __uv$config) {
        super();
        if (!config.bare) config.bare = '/book/';
    ...
    async fetch({ request }) {
      try {
        if (!request.url.startsWith(location.origin + (this.config.prefix || '/math/'))) {
        ...
        try {
            ...
            if (typeof this.config.construct === 'function') {
                this.config.construct(ultraviolet, 'math');
            };
